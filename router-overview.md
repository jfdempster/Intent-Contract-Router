...
The repository-local implementation is the enforcement boundary:

- `src/validate.ts` performs schema, invariant, evidence occurrence, coverage, and semantic-link checks.
- `src/router.ts` applies R1-R5 in order after validation only.
- `src/operation.ts` loads exactly one canonical prompt and refuses invalid records.
- `src/envelope.ts` assembles the mandatory seven-section envelope.
